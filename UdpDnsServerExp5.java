    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            for (int i = 0; i < hosts.length; i++)
                if (hosts[i].equalsIgnoreCase(host)) { response = ips[i]; break; }
            serverSocket.send(new DatagramPacket(response.getBytes(), response.length(), request.getAddress(), request.getPort()));
        }
    }
}